...
//   }
// }
// src/app/api/produtos/lote/edit/route.js
import { NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '../../../auth/[...nextauth]/route'
import prisma from '@/app/lib/prisma'
...
export async function POST(request) {
  try {
    ...
    const { lote, precoVenda, precoCusto, emPromocao, precoPromocao } = await request.json()
    if (!lote?.trim()) return NextResponse.json({ error: 'Código do lote obrigatório' }, { status: 400 })

    let dataToUpdate = {}

    if (precoVenda !== undefined) {
      const pv = parseFloat(precoVenda)
      if (isNaN(pv) || pv <= 0) return NextResponse.json({ error: 'Preço de venda inválido' }, { status: 400 })
      dataToUpdate.precoVenda = pv
    }

    if (precoCusto !== undefined) {
      const pc = parseFloat(precoCusto)
      if (isNaN(pc) || pc < 0) return NextResponse.json({ error: 'Preço de custo inválido' }, { status: 400 })
      dataToUpdate.precoCusto = pc
    }

    if (emPromocao !== undefined) {
      dataToUpdate.emPromocao = !!emPromocao
      if (emPromocao) {
        if (!precoPromocao) return NextResponse.json({ error: 'Preço promocional obrigatório' }, { status: 400 })
        const pp = parseFloat(precoPromocao)
        if (isNaN(pp) || pp <= 0 || (precoVenda !== undefined && pp >= precoVenda)) {
          return NextResponse.json({ error: 'Preço promocional inválido' }, { status: 400 })
        }
        dataToUpdate.precoPromocao = pp
      } else {
    ...
    if (result.count === 0) return NextResponse.json({ error: 'Lote não encontrado' }, { status: 404 })

    return NextResponse.json({
      message: `Lote atualizado: ${result.count} produtos alterados`,
